...
import { Mic, ShoppingCart, X, Plus, Minus, Loader2, Search, Volume2, MapPin, Banknote, CheckCircle, ArrowLeft } from 'lucide-react';
...
export default function App() {
  ...
  const [displayedProducts, setDisplayedProducts] = useState<Product[]>(CATALOG);
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      setDisplayedProducts(CATALOG);
    } else {
      const lower = searchQuery.toLowerCase();
      setDisplayedProducts(CATALOG.filter(p => p.name.toLowerCase().includes(lower) || p.category.toLowerCase().includes(lower)));
    }
  }, [searchQuery]);
  ...
  const startListening = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      if (final_transcript) {
        ...
        handleTranscript(final_transcript, true);
      }
      
      interimTranscriptRef.current = interim_transcript;
      setTranscript(fullTranscriptRef.current + interim_transcript);
      resetSilenceTimer();
    };
    ...
    recognition.onend = () => {
      ...
      if (interimTranscriptRef.current.trim()) {
        ...
        handleTranscript(interimTranscriptRef.current, false);
        interimTranscriptRef.current = '';
      } else {
        setIsCartOpen(true);
      }
  ...
  const handleTranscript = async (text: string, isRealTime: boolean = false) => {
    if (!text.trim()) return;
      ...
      if (extracted && extracted.length > 0) {
        ...
        extracted.forEach((item: any) => {
          ...
          if (item.action === 'remove') {
            ...
            if (!addedNames.includes(product.name)) addedNames.push(product.name);
          }
        });
        ...
        if (!isRealTime) {
          speak("Sorry, I didn't catch any grocery items.");
        }
      }
    } finally {
      setProcessingCount(prev => prev - 1);
  ...
  const closeSidebar = () => {
    setIsCartOpen(false);
    setTimeout(() => setCartView('cart'), 300); // Reset view after animation
  };
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center mb-8">
          ...
          <div className="mt-6 relative">
            <form onSubmit={(e) => {
              e.preventDefault();
              if (searchQuery.trim()) {
                handleTranscript(searchQuery, false);
                setSearchQuery('');
              }
            }}>
          ...
          {displayedProducts.map(product => (
            <div key={product.id} className="bg-white rounded-2xl p-3 shadow-sm border border-gray-100 flex flex-col">
              <div className="relative aspect-square bg-gray-50 rounded-xl flex items-center justify-center text-5xl mb-3">
                {product.emoji}
                <div className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-1 rounded-md ${
                ...
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      {address ? (
                        <div className="bg-purple-50 p-3 rounded-xl border border-purple-100 text-sm text-purple-900 mb-3">
                          {address}
                        </div>
                      ) : (
                        <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 text-sm text-gray-500 mb-3">
                          No address selected
                        </div>
                      )}
